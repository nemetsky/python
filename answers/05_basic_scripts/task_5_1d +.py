...
=== Мое решение (такое же только с промежуточной переменной)

name = input("Введите имя устройства: ")
keys = ",".join(london_co[name].keys())
parametr = input(f"Введите имя параметра ({keys}): ")
parametr_2 = parametr.lower()
print(london_co[name].get(parametr_2, "Нет такого ключа"))
