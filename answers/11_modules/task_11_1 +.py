...
# решение похожее, только условие проверки строки, которая показывает соседа, у меня другое 
# elif s[-1:].isdigit():    - проверяет если строка заканчивается на цифру, то это нужная строка
# у автора мне кажется универсальней решение (хотя не факт), она разбивает строку на список и если в списке более 5 элементов и есть цифры вс в 4-ом столбце то это нужная строка
