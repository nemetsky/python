...
def send_show_command(device, commands):
    ...
    with ConnectHandler(**device) as ssh:
        ssh.enable()
        for command in commands:                            # автор тут отправляет список команд на устройство в цикле, один раз подключившись к нему и суммирует вывод
...
# ========================================================================================================================
# Мое решение #      (Сделал по другому, вернее неправильно, ни как у автора, хотя результат такой же)
#                     в моем скрипте, чтобы отправить несколько команд на устройство (список команд), надо столько же раз к ней подключиться
#                     автор один раз подключается на устройство, и оправляет в цикле команды                      
...
def send_show_command(device, password, command):
    ...
    try:
        with netmiko.ConnectHandler(password=password, **device) as ssh:             # !!! надо было в этой функции перебирать команды, которые отправляем на устройство
...
def send_command_to_devices(devices, commands_dict, filename, limit=3):
    ...
    with ThreadPoolExecutor(max_workers=limit) as executor:
        for dev in devices:
            for command in commands_dict[dev["host"]]:                               # !!! А я перебираю команды из списка здесь, (а автор в другой функции сделал этот цикл)
                future = executor.submit(send_show_command, dev, password, command)  # тем самым для отправки каждой команды мне надо вызвать executor, т.е. подключаться к оборудованию каждый раз
