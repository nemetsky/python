...
'''
def check_passwd(username, password, *, min_len=8, check_numbers=False):        # добавили "*,", чтобы обязательно необязательный параметры передавать как ключевые аргументы
    """
...
# check_passwd("user1", "1234", 4, True)
...
# Пример 4  (c ключевыми аргументами переменной длины (**kwargs))


def check_passwd(username, password, *, min_len=8, check_numbers=False):    # теперь можем добавлять сюда ключевых аргументов и не надо в функции check_user_list что-либо менять
    if len(password) < min_len:
        return False
    elif username.lower() in password.lower():                      
        return False
...
def check_user_list(user_passwd_data, **kwargs):       # добавили ключевыми аргументами переменной длины (запаковка ключевых аргументов в словарь)       
   # print(f"{kwargs=}")
    correct_users = []                     
    wrong_users = []
    for username, password in user_passwd_data:     
        check = check_passwd(username, password, **kwargs)   # в самой этой функции check_user_list их не используем, 
        if check:                                      # но передаем их в функцию check_passwd (распаковка словаря в ключевые аргументы)
...
ok, no_ok = check_user_list(data, min_len=4, check_numbers=True)    # передаем ключевые аргументы здесь, чтобы проверить          
print(ok)                   
print(no_ok)
