...
и с помощью регулярок найти множество портов между которыми флаппинг
"""
...
with open("log.txt") as f:
    for line in f:
        ...
        if match:                         # обязательно, так как если совпадения нет, то результат None, а None нет метода groups, поэтому будет ошибка
...
задачу которую решали раньше (сделать список из вывода show_ip_int_br), сделать через регулярки:
...
result_list = []
regex = r"(\S+) +([\d.]+) +\S+ +\w+ +(up|down) +(up|down)"             # регяркой описали 6 столбцов и 4 запомнили из них
...
[['FastEthernet0/0', '15.0.15.1', 'up', 'up'],
 ...
 ['FastEthernet0/3', 'unassigned', 'up', 'down'],                 # ([\d.]+)  - в такой регулярке не попадет эта строка
...
"""
Задача 3 (это Задача 2 через функцию)
"""
...
"""
Задача 4 (это Задача 3, только используем словарь)
"""
