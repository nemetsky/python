...
def send_command(device, password, commands_cisco, commands_eltex):
    ip = device["host"]
    logging.info(f"===> Connection: {ip}")
    if device["device_type"] == "cisco_ios":
        commands = commands_cisco.copy()
    elif device["device_type"] == "eltex":
        commands = commands_eltex.copy()
        commands.insert(1, f"backup path /{device['host']}.txt")
    try:
        with netmiko.ConnectHandler(password=password, **device) as ssh:
            result = ssh.send_config_set(commands)
            #result = ssh.send_config_from_file('commands_eltex.txt')
...
def send_config_commands(devices, commands_cisco, commands_eltex, log_files, max_threads=8):
    password = getpass.getpass(prompt="Введите пароль: ")
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        future_list = []
        for device in devices:
            future = executor.submit(send_command, device, password, commands_cisco, commands_eltex)
...
if __name__ == "__main__":
    with open("commands_eltex.txt") as f:
        lines_eltex = f.readlines()
        commands_eltex = [line.rstrip() for line in lines_eltex]
    with open("commands_cisco.txt") as f:
        lines_cisco = f.readlines()
        commands_cisco = [line.rstrip() for line in lines_cisco]
    with open("devices_sw.yaml") as f:
        devices = yaml.safe_load(f)
    result_dict = send_config_commands(devices, commands_cisco, commands_eltex, "logs_{}/log_{}.txt")
